...
function Tweet({ tweet, user}) {
	const [likes, setLikes] = useState(tweet.likes);
	  ...
	  const handleLike = async () => {
		...
		if (liked) {
		  const response = await axios.delete(`http://localhost:8000/tweets/${tweet._id}/likes`, { data: { user} });
		  dispatch(setLikes(response.data.likes));
		  setLiked(false);
		} else {

		  const response = await axios.post(`http://localhost:8000/tweets/${tweet._id}/likes`, { data: { user } });
		  dispatch(setLikes(response.data.likes));
		  setLiked(true);
		}
	  };
							...
							<div class="d-flex justify-content-between">
								<div id="interact-icon">
									<ul id="interaction-list">
										...
										<button className="btn btn-danger" onClick={handleLike}>{liked ? 'No me gusta' : 'Me gusta'} ({likes.length})</button>
											<span></span>
										</li>
										<li id="comment-icon">
